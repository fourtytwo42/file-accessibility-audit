...
describe('audit config', () => {
  ...
  it('uses fast remediation guardrails tuned for shorter repair loops', () => {
    ...
    expect(REMEDIATION.TOOL_RELIABILITY_DEFAULT).toBe(0.85)
    expect(REMEDIATION.TOOL_RELIABILITY_MIN_ATTEMPTS).toBe(3)
    expect(REMEDIATION.EARLY_EXIT_SCORE_THRESHOLD).toBe(98)
    expect(REMEDIATION.NET_BENEFIT_MAX_CATEGORY_REGRESSION).toBe(5)
