...
const waveManifestPath = path.join(manifestsRoot, 'stage6-long-report-wave.json')
const waveOutcomesPath = path.join(manifestsRoot, 'stage6-long-report-wave.outcomes.json')
const waveOutcomesSummaryPath = path.join(manifestsRoot, 'stage6-long-report-wave.outcomes.summary.json')
...
export async function main(): Promise<void> {
  ...
  const buildEnv = pendingOnly
    ? {
        ...process.env,
        ICJIA_STAGE6_LONG_REPORT_WAVE_INCLUDE_IDS: readPendingPublicationIds().join(','),
      }
    : process.env

  run('pnpm', ['agency:build-stage6-long-report-wave'], buildEnv)
  const selectedRows = readWaveSelectedCount()
  if (pendingOnly && !readPendingPublicationIds().length) {
    console.log(JSON.stringify({ skipped: true, reason: 'No pending Stage 6 rows remain.' }, null, 2))
    return
  }
  if (!selectedRows) {
    console.log(JSON.stringify({ skipped: true, reason: 'No Stage 6 long-report-wave candidates selected.' }, null, 2))
    return
  }
  ...
  if (!buildOnly) {
    run('pnpm', ['agency:run-priority-remediation'], {
      ...
      ICJIA_REMEDIATION_DETAILED_REPORT_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'test-runs', 'stage6-long-report-wave'),
      ICJIA_REMEDIATION_FAILURE_REPORT_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'failures', 'stage6-long-report-wave'),
      ICJIA_REMEDIATION_REMEDIATED_PDF_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'artifacts', 'remediated-pdfs', 'stage6-long-report-wave'),
      ICJIA_REMEDIATION_ATTEMPTS_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'artifacts', 'remediation-attempts', 'stage6-long-report-wave'),
    })
  ...
  run('pnpm', ['agency:analyze-stage6-long-report-forensics'])
  run('pnpm', ['agency:build-stage6-long-report-wave'])
}
