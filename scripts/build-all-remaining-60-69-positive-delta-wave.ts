...
type WaveManifest = {
  ...
  laneName: 'all-remaining-60-69-positive-delta-wave'
  laneIntent: 'pass_rate_conversion'
  ...
  totals: {
    ...
    excludedFlatOrNegativeDelta: number
  }
...
const outputManifestPath = path.join(manifestsRoot, 'all-remaining-60-69-positive-delta-wave.json')
const outputSummaryPath = path.join(manifestsRoot, 'all-remaining-60-69-positive-delta-wave.summary.json')
...
function derivePriorityTier(delta: number, currentScore: number): 'highest' | 'high' | 'medium' {
  if (delta >= 8 || currentScore >= 68) return 'highest'
  if (delta >= 3 || currentScore >= 66) return 'high'
  return 'medium'
}
...
function toCandidate(outcome: PriorOutcome): WaveCandidate {
  ...
  const scoreDelta = seedScore == null ? 0 : finalScore - seedScore
  ...
  const blockingFindingKeys = Array.isArray(outcome.gate?.blockingLocalFindingKeys) && outcome.gate?.blockingLocalFindingKeys.length
    ...
    : Array.isArray(outcome.blockingFindingKeys)
      ...
      : []
  const priorityTier = derivePriorityTier(scoreDelta, finalScore)

  return {
    priorityRank: 0,
    passLikelihoodScore: Math.max(0, Math.min(100, finalScore + scoreDelta)),
    priorityTier,
    recommendedAction: 'fix_first',
...
export function buildManifest(source: PriorManifest): WaveManifest {
  ...
  let excludedFlatOrNegativeDelta = 0

  const selected = sourceOutcomes
    .filter(outcome => {
      const score = outcome.final?.overallScore
      const priorScore = outcome.seed?.priorOverallScore
      ...
      if (!Number.isFinite(priorScore) || Number(score) <= Number(priorScore)) {
        excludedFlatOrNegativeDelta += 1
        return false
      }
  ...
  return {
    ...
    sourceReportRoot: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'test-runs', 'all-remaining-60-69-positive-delta-wave'),
    laneName: 'all-remaining-60-69-positive-delta-wave',
    laneIntent: 'pass_rate_conversion',
    executionPolicy: 'active',
    executionPolicyReason: 'run_only_current_60_69_rows_that_still_show_positive_score_delta',
