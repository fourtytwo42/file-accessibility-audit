...
const laneName = 'all-remaining-dropped-rerun'
...
export async function main(): Promise<void> {
  ...
  if (!resumeExisting) {
    run('pnpm', ['agency:build-all-remaining-dropped-rerun'])
    if (!fs.existsSync(manifestPath)) {
      throw new Error(`Missing dropped-rerun manifest after build: ${manifestPath}`)
    }
    resetCampaignState()
  } else {
