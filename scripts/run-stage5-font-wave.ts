...
const waveManifestPath = path.join(manifestsRoot, 'stage5-font-wave.json')
const waveOutcomesPath = path.join(manifestsRoot, 'stage5-font-wave.outcomes.json')
const waveOutcomesSummaryPath = path.join(manifestsRoot, 'stage5-font-wave.outcomes.summary.json')
...
export async function main(): Promise<void> {
  ...
  const buildEnv = pendingOnly
    ? {
        ...process.env,
        ICJIA_STAGE5_FONT_WAVE_INCLUDE_IDS: readPendingPublicationIds().join(','),
      }
    : process.env

  run('pnpm', ['agency:build-stage5-font-wave'], buildEnv)
  const selectedRows = readWaveSelectedCount()
  if (pendingOnly && !readPendingPublicationIds().length) {
    console.log(JSON.stringify({ skipped: true, reason: 'No pending Stage 5 rows remain.' }, null, 2))
    return
  }
  if (!selectedRows) {
    console.log(JSON.stringify({ skipped: true, reason: 'No Stage 5 font-wave candidates selected.' }, null, 2))
    return
  }
  ...
  if (!buildOnly) {
    run('pnpm', ['agency:run-priority-remediation'], {
      ...
      ICJIA_REMEDIATION_DETAILED_REPORT_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'test-runs', 'stage5-font-wave'),
      ICJIA_REMEDIATION_FAILURE_REPORT_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'failures', 'stage5-font-wave'),
      ICJIA_REMEDIATION_REMEDIATED_PDF_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'artifacts', 'remediated-pdfs', 'stage5-font-wave'),
      ICJIA_REMEDIATION_ATTEMPTS_ROOT: path.join(repoRoot, 'ICJIA-PDFs', 'artifacts', 'remediation-attempts', 'stage5-font-wave'),
    })
  ...
  run('pnpm', ['agency:analyze-stage5-font-forensics'])
  run('pnpm', ['agency:build-stage5-font-wave'])
}
