...
const laneName = 'remediated-pdf-grading-sub80-rerun-v2'
...
export async function main(): Promise<void> {
  ...
  if (!resumeExisting) {
    run('pnpm', ['agency:build-remediated-pdf-grading-sub80-rerun-v2'])
    if (!fs.existsSync(manifestPath)) {
      throw new Error(`Missing sub80 rerun v2 manifest after build: ${manifestPath}`)
    }
    resetCampaignState()
  } else {
