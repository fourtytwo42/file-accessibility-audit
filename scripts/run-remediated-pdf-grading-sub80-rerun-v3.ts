...
const laneName = 'remediated-pdf-grading-sub80-rerun-v3'
...
export async function main(): Promise<void> {
  ...
  if (!resumeExisting) {
    run('pnpm', ['agency:build-remediated-pdf-grading-sub80-rerun-v3'])
    if (!fs.existsSync(manifestPath)) {
      throw new Error(`Missing sub80 rerun v3 manifest after build: ${manifestPath}`)
    }
    resetCampaignState()
  } else {
  ...
  run('pnpm', ['agency:run-priority-remediation'], {
    ...
    SEMANTIC_REQUEST_CONCURRENCY: '1',
