...
type OutputManifest = {
  ...
  laneName: 'remediated-pdf-grading-sub80-rerun-v3'
  laneIntent: 'pass_rate_conversion'
...
const sourceOutcomesPath = path.join(manifestsRoot, 'remediated-pdf-grading-sub80-rerun-v2.outcomes.json')
const outputManifestPath = path.join(manifestsRoot, 'remediated-pdf-grading-sub80-rerun-v3.json')
const outputSummaryPath = path.join(manifestsRoot, 'remediated-pdf-grading-sub80-rerun-v3.summary.json')
...
function toCandidate(outcome: PriorOutcome): RerunCandidate {
  ...
  const pageCount = safeNumber(outcome.final?.pageCount, 0)
  ...
  return {
    ...
    heuristicReasons: [
      'rerun_remediated_sub80_rerun_v3',
      'source:remediated-pdf-grading-sub80-rerun-v2',
...
export function buildManifest(source: PriorManifest): OutputManifest {
  ...
  return {
    ...
    sourceReportRoot: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'test-runs', 'remediated-pdf-grading-sub80-rerun-v2'),
    laneName: 'remediated-pdf-grading-sub80-rerun-v3',
    laneIntent: 'pass_rate_conversion',
    executionPolicy: 'active',
    executionPolicyReason: 'Fresh rerun completed with retained sub-80 outputs that should be tested one more time for additional lift.',
...
export async function main(): Promise<void> {
  ...
  console.log(JSON.stringify({
    ...
    excludedAbove79: manifest.totals.excludedAbove79,
    excludedNonTerminal: manifest.totals.excludedNonTerminal,
    excludedMissingArtifact: manifest.totals.excludedMissingArtifact,
