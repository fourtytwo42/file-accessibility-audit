...
const laneName = 'remediated-pdf-grading-sub80-rerun'
...
export async function main(): Promise<void> {
  ...
  if (!resumeExisting) {
    run('pnpm', ['agency:build-remediated-pdf-grading-sub80-rerun'])
    if (!fs.existsSync(manifestPath)) {
      throw new Error(`Missing grading sub80 rerun manifest after build: ${manifestPath}`)
    }
    resetCampaignState()
  } else {
