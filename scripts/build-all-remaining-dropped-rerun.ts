...
type RerunCandidate = {
  ...
  seedResultBand: 'drop_fail_lt_80'
...
type RerunManifest = {
  ...
  laneName: 'all-remaining-dropped-rerun'
  laneIntent: 'pass_rate_conversion'
...
const sourceOutcomesPath = path.join(manifestsRoot, 'all-remaining-automated.outcomes.json')
const outputManifestPath = path.join(manifestsRoot, 'all-remaining-dropped-rerun.json')
const outputSummaryPath = path.join(manifestsRoot, 'all-remaining-dropped-rerun.summary.json')
...
function toCandidate(outcome: PriorOutcome): RerunCandidate {
  ...
  return {
    ...
    localCachePath: outcome.localCachePath ?? null,
...
export function buildManifest(source: PriorManifest): RerunManifest {
  ...
  let excludedNonDropRows = 0
  let excludedMissingSource = 0
  let excludedMissingRemote = 0

  const selected = sourceOutcomes
    .filter(outcome => {
      if (outcome.resultBand !== 'drop_fail_lt_80' || outcome.status !== 'failed_after_remediation') {
        excludedNonDropRows += 1
        return false
      }
      if (!outcome.localCachePath || !fs.existsSync(outcome.localCachePath)) {
        excludedMissingSource += 1
        return false
      }
  ...
  return {
    ...
    sourceReportRoot: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'test-runs', 'all-remaining-dropped-rerun'),
    laneName: 'all-remaining-dropped-rerun',
    laneIntent: 'pass_rate_conversion',
    executionPolicy: 'active',
    executionPolicyReason: 'rerun_prior_drop_lt_80_with_full_artifact_retention',
...
export async function main(): Promise<void> {
  if (!fs.existsSync(sourceOutcomesPath)) {
    throw new Error(`Missing source outcomes manifest: ${sourceOutcomesPath}`)
  }

  const manifest = buildManifest(readJson<PriorManifest>(sourceOutcomesPath))
  const scores = manifest.candidates.map(candidate => candidate.seedOverallScore).filter((value): value is number => Number.isFinite(value))
  const summary = {
    generatedAt: manifest.generatedAt,
    basedOn: sourceOutcomesPath,
    ...
    priorScoreBands: {
      ...
      '40-49': scores.filter(score => score >= 40 && score < 50).length,
      '<40': scores.filter(score => score < 40).length,
    },
    topSingleBlockerRows: manifest.candidates
      .filter(candidate => candidate.blockingFindingKeys.length === 1)
      .slice(0, 25)
      .map(candidate => ({
        publicationId: candidate.publicationId,
        publicationTitle: candidate.publicationTitle,
        priorOverallScore: candidate.seedOverallScore,
        blocker: candidate.blockingFindingKeys[0] || null,
        runtimeWeightBucket: candidate.runtimeWeightBucket,
      })),
  }

  writeJson(outputManifestPath, manifest)
  writeJson(outputSummaryPath, summary)
