...
MANIFEST_PATHS = [
    REPO_ROOT / 'ICJIA-PDFs/manifests/all-remaining-79-down-rerun.outcomes.json',
    REPO_ROOT / 'ICJIA-PDFs/manifests/all-remaining-dropped-rerun.outcomes.json',
    REPO_ROOT / 'ICJIA-PDFs/manifests/all-remaining-automated.outcomes.json',
    REPO_ROOT / 'ICJIA-PDFs/manifests/remediation-batch-outcomes.json',
]
REPORT_ROOT = REPO_ROOT / 'ICJIA-PDFs/reports/visual-parity'
DETAIL_PATH = REPORT_ROOT / 'remediated-80-plus-visual-parity.json'
SUMMARY_PATH = REPORT_ROOT / 'remediated-80-plus-visual-parity.summary.json'
RENDER_DPI = 96
...
def main() -> None:
    pairs = load_pairs()
    workspace = Path(tempfile.mkdtemp(prefix='pdf-visual-parity-80-plus-'))
