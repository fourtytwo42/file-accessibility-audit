...
import { buildStage6LongReportForensicsArtifacts } from '../apps/api/src/services/longReportStage6.ts'
...
export async function main(): Promise<void> {
  ...
  if (!validation.ok) {
    throw new Error('Cannot build Stage 6 long-report forensics from an invalid control plane.')
  }

  const includePublicationIds = (process.env.ICJIA_STAGE6_LONG_REPORT_FORENSICS_INCLUDE_IDS || '')
    .split(',')
  ...
  const summaryPath = path.join(sources.manifestsRoot, 'stage6-long-report-forensics.summary.json')
  writeJson(forensicsPath, forensics.document)
  writeJson(summaryPath, forensics.summary)

  console.log(JSON.stringify({
    forensicsPath,
    summaryPath,
    includePublicationIds,
    totals: forensics.summary.totals,
    publicationIdsByDisposition: forensics.summary.publicationIdsByDisposition,
    stillUnclassifiedPublicationIds: forensics.summary.stillUnclassifiedPublicationIds,
  }, null, 2))
}
