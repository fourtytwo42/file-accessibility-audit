...
import { buildStage5FontForensicsArtifacts } from '../apps/api/src/services/fontWaveStage5.ts'
...
export async function main(): Promise<void> {
  ...
  if (!validation.ok) {
    throw new Error('Cannot build Stage 5 font forensics from an invalid control plane.')
  }

  const includePublicationIds = (process.env.ICJIA_STAGE5_FONT_FORENSICS_INCLUDE_IDS || '')
    .split(',')
  ...
  const summaryPath = path.join(sources.manifestsRoot, 'stage5-font-forensics.summary.json')
