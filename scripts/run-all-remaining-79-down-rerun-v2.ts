...
const laneName = 'all-remaining-79-down-rerun-v2'
...
export async function main(): Promise<void> {
  ...
  if (!resumeExisting) {
    run('pnpm', ['agency:build-all-remaining-79-down-rerun-v2'])
    if (!fs.existsSync(manifestPath)) {
      throw new Error(`Missing 79-down rerun v2 manifest after build: ${manifestPath}`)
    }
    resetCampaignState()
  } else {
