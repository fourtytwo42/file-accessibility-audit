...
const laneName = 'all-remaining-79-down-rerun'
...
export async function main(): Promise<void> {
  ...
  if (!resumeExisting) {
    run('pnpm', ['agency:build-all-remaining-79-down-rerun'])
    if (!fs.existsSync(manifestPath)) {
      throw new Error(`Missing 79-down rerun manifest after build: ${manifestPath}`)
    }
    resetCampaignState()
  } else {
