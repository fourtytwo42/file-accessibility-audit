...
type RerunManifest = {
  ...
  laneName: 'all-remaining-79-down-rerun'
  laneIntent: 'pass_rate_conversion'
...
const sourceOutcomesPath = path.join(manifestsRoot, 'all-remaining-dropped-rerun.outcomes.json')
const outputManifestPath = path.join(manifestsRoot, 'all-remaining-79-down-rerun.json')
const outputSummaryPath = path.join(manifestsRoot, 'all-remaining-79-down-rerun.summary.json')
...
function toCandidate(outcome: PriorOutcome): RerunCandidate {
  ...
  return {
    ...
    heuristicReasons: [
      'rerun_prior_79_down',
      'source:all-remaining-dropped-rerun',
...
export function buildManifest(source: PriorManifest): RerunManifest {
  ...
  return {
    ...
    sourceReportRoot: path.join(repoRoot, 'ICJIA-PDFs', 'reports', 'test-runs', 'all-remaining-79-down-rerun'),
    laneName: 'all-remaining-79-down-rerun',
    laneIntent: 'pass_rate_conversion',
    executionPolicy: 'active',
    executionPolicyReason: 'rerun_prior_79_down_outputs_with_full_artifact_retention',
