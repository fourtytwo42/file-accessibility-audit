...
- **Canonical Ubuntu 24.04 VM path for API + local Gemma (2026-04-13):** fresh local-AI hosts should now use **`bash scripts/provision-vm.sh`** followed by **`bash scripts/start-stack.sh`** as the supported path after `git clone`. Provisioning reuses **`bootstrap-ubuntu.sh`**, writes/preserves local `OPENAI_COMPAT_*` defaults in **`apps/api/.env`**, installs a pinned llama.cpp bundle when missing, installs repo-managed systemd units **`pdfaf-llm.service`** and **`pdfaf-api.service`**, and can run **`scripts/healthcheck-stack.sh`** as a post-install check. Operator helpers: **`scripts/stop-stack.sh`**, **`scripts/status-stack.sh`**, **`scripts/healthcheck-stack.sh`** plus package aliases **`pnpm vm:provision`**, **`pnpm stack:start|stop|status|healthcheck`**.
